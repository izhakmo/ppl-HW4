...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, isLetValuesExp, LetValues, Values, BindingValues } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, SExpValue, makeTuple, isSValues, SValues, isSExp } from "./L5-value";
...
const evalLetValues = (exp: LetValues, env: Env): Result<Value> => {
    const moshe = mapResult((x) => bindValuesEvaluator(x,env) , exp.bindings);
    const mosheValsnotFlatten = bind(moshe,(y) :Result<SExpValue[][] > =>  mapResult((x) =>isSValues(x) ? makeOk(x.vals) :makeFailure(`${y}`) ,y));
    const mosheVals = bind(mosheValsnotFlatten ,x=> makeOk(flatten(x)))
    const mosheVarsnotFlatten = flatten(map((x ) => x.var,exp.bindings));
    const mosheVars = makeOk(map((x ) => x.var,mosheVarsnotFlatten))
    return safe2((vars :string[], vals :SExpValue[]) => evalSequence(exp.body,makeExtEnv(vars,vals,env)))
        (mosheVars,mosheVals)
}


const bindValuesEvaluator = (exp: BindingValues,env: Env) : Result<Value> =>{
    const tuple = applicativeEval(exp.val,env);
    return bind(tuple,x => isSValues(x) && x.vals.length === exp.var.length ? makeOk(x) : makeFailure(`wrong lengths : tuple ${x} ::: vars ${exp.var.length}`) )
}


const evalValues = (exp: Values, env: Env): Result<Value> => {      
    const moshe = mapResult((x ) =>applicativeEval(x,env) ,exp.tuple);
    return bind(moshe,(x => makeOk(makeTuple(x))))
}
